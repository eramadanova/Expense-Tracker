...
def extract_and_validate_category(row, index):
    """
...
def extract_and_validate_description(row):
    """
...
def extract_and_validate_amount(row, index):
    """
    ...
    try:
        ...
        raise ValueError(f'Amount \'{amount}\' is not a valid number on row {index+1}') from exc

    return float(amount)

def extract_and_validate_date(row, index):
    """
...
def extract_and_validate_currency(row, index):
    """
